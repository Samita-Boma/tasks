...
export function getPublishedQuestions(questions: Question[]): Question[] {
    return questions.filter((quest) => quest.published);
}
...
export function findQuestion(
    ...
    const found = questions.find((quest) => quest.id === id);
    if (found) {
        return found;
    }
    return null;
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    return questions.filter((quest) => quest.id !== id);
}
...
export function getNames(questions: Question[]): string[] {
    return questions.map((quest) => quest.name);
}
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce((sum, quest) => sum + quest.points, 0);
}
...
export function publishAll(questions: Question[]): Question[] {
    return questions.map((quest) => ({ ...quest, published: true }));
}
...
export function sameType(questions: Question[]): boolean {
    ...
    return questions.every((quest) => quest.type === first);
}
